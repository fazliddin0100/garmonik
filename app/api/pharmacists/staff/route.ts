...
function pharmacistInputFromBody(body: Record<string, unknown>) {
  return {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const input = pharmacistInputFromBody(body);
    const item = id
      ? await updatePharmacistStaffAccount({ id, ...input })
      : await createPharmacistStaffAccount(input);
