...
import { isAllowedPortalAdminCreationRole } from '@/lib/admins/roles';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isAllowedPortalAdminCreationRole(roleName)) {
      return NextResponse.json(
        {
          error:
            'Bu rol bilan portal hisobi yaratib bo‘lmaydi. Kassir, Buxgalter yoki boshqa ma’muriy rolni tanlang.',
        },
        { status: 400 },
      );
    }
