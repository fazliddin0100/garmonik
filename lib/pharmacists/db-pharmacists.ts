...
import {
  normalizePharmacistRow,
  pharmacistToClinicJson,
  type PharmacistRow,
...
export type PharmacistStaffInput = {
  id?: string;
  fullName: string;
  specialty: string;
  department?: string;
...
function rowFromProfile(r: PortalProfileRow): PharmacistRow {
  const active = r.is_active;
  return {
    id: r.legacy_external_id || r.user_id,
    code: '',
    fullName: r.display_name,
    specialty: r.role_label?.trim() || 'Farmatsevt',
    degree: '',
    department: r.department?.trim() || '',
    contact: '',
    login: (r.staff_login || r.auth_email).toLowerCase(),
    status: active ? 'Актив' : 'Nofaol',
  };
}

async function syncClinicJsonRow(row: PharmacistRow): Promise<void> {
  const clinicId = await getDefaultClinicId();
  const payload = await readClinicResourcePayload(clinicId, 'pharmacists');
  const existing = Array.isArray(payload) ? payload : [];
  const normalized = existing
    .map(normalizePharmacistRow)
    .filter((x): x is PharmacistRow => x !== null);
  const without = normalized.filter((p) => p.id !== row.id);
  await upsertClinicResourcePayload(clinicId, 'pharmacists', [
    pharmacistToClinicJson(row),
    ...without.map(pharmacistToClinicJson),
  ]);
}
...
async function removeClinicJsonRow(id: string): Promise<void> {
  ...
  const normalized = existing
    .map(normalizePharmacistRow)
    .filter((x): x is PharmacistRow => x !== null && x.id !== id);
  await upsertClinicResourcePayload(
    clinicId,
    'pharmacists',
    normalized.map(pharmacistToClinicJson),
  );
}

export async function listPharmacistStaffAccounts(): Promise<PharmacistRow[]> {
  const clinicId = await getDefaultClinicId();
...
export async function createPharmacistStaffAccount(
  input: PharmacistStaffInput,
): Promise<PharmacistRow> {
  ...
  await insertPortalProfile({
    ...
    staff_role: 'farmatsevt',
    display_name: input.fullName.trim(),
    role_label: input.specialty.trim() || 'Farmatsevt',
    staff_login: login,
  ...
  const row: PharmacistRow = {
    id,
    code: '',
    fullName: input.fullName.trim(),
    specialty: input.specialty.trim() || 'Farmatsevt',
    degree: '',
    department: (input.department ?? '').trim(),
    contact: '',
    login,
    status: (input.isActive ?? true) ? 'Актив' : 'Nofaol',
  };
  await syncClinicJsonRow(row);
  return row;
}

export async function updatePharmacistStaffAccount(
  input: PharmacistStaffInput & { id: string },
): Promise<PharmacistRow> {
  const login = input.login.trim().toLowerCase();
  if (!input.fullName.trim() || !login) {
    throw new Error('F.I.SH va login majburiy');
  ...
  if (!profile || profile.staff_role !== 'farmatsevt') {
...
export async function deletePharmacistStaffAccount(id: string): Promise<void> {
  const profile = await findStaffProfileByExternalId(id);
  if (profile?.staff_role === 'farmatsevt') {
