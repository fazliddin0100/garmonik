...
import {
  createEmptyDutyDoctorExamination,
  DUTY_DOCTOR_EXAM_FIELD_LABELS,
  DUTY_DOCTOR_EXAM_TEXT_FIELDS,
  formatDutyDoctorExaminationSavedAt,
  formatDutyDoctorExaminationTitle,
  getPatientDutyDoctorExaminations,
  type PatientDutyDoctorExamination,
...
type PatientDutyDoctorExamPanelProps = {
  patient: PatientRow;
  saving?: boolean;
  defaultDoctorUserId?: string | null;
  onSave: (input: PatientDutyDoctorExaminationInput) => void | Promise<void>;
};
...
function examToInput(exam: PatientDutyDoctorExamination): PatientDutyDoctorExaminationInput {
  return {
    id: exam.id,
...
function buildNewDraft(
  ...
  return createEmptyDutyDoctorExamination({
    receptionDoctorUserId: defaultDoctor?.id ?? '',
    receptionDoctorName: defaultDoctor?.fullName ?? '',
  });
}

export default function PatientDutyDoctorExamPanel({
  patient,
  saving = false,
  defaultDoctorUserId = null,
  onSave,
}: PatientDutyDoctorExamPanelProps) {
  const records = useMemo(() => getPatientDutyDoctorExaminations(patient), [patient]);
  const [mode, setMode] = useState<PanelMode>('compose');
  const [editingId, setEditingId] = useState<string | null>(null);
  const [viewId, setViewId] = useState<string | null>(null);
  const [doctors, setDoctors] = useState<DoctorOption[]>([]);
  const [draft, setDraft] = useState<PatientDutyDoctorExaminationInput>(() =>
    createEmptyDutyDoctorExamination(),
  );
  ...
  function openEditForm(record: PatientDutyDoctorExamination) {
  ...
  function openView(record: PatientDutyDoctorExamination) {
    setViewId(record.id);
    setMode('view');
  }

  function updateDraft(patch: Partial<PatientDutyDoctorExaminationInput>) {
  ...
  async function handleSubmit() {
    ...
    const payload: PatientDutyDoctorExaminationInput = {
      ...draft,
    ...
    return (
      <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
        <div className="mb-5 flex flex-wrap items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 text-slate-800">
              ...
              <h3 className="font-semibold">Navbatchi shifokor ko&apos;rigi</h3>
            </div>
            <p className="text-sm text-slate-500">
              {formatDutyDoctorExaminationTitle(activeRecord)}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <FieldBlock
            label={DUTY_DOCTOR_EXAM_FIELD_LABELS.examinationDate}
            value={activeRecord.examinationDate}
            multiline={false}
          />
          <FieldBlock
            label={DUTY_DOCTOR_EXAM_FIELD_LABELS.examinationTime}
            value={activeRecord.examinationTime}
            multiline={false}
          />
          <div className="md:col-span-2">
            <FieldBlock
              label={DUTY_DOCTOR_EXAM_FIELD_LABELS.receptionDoctor}
              value={activeRecord.receptionDoctorName}
              multiline={false}
            />
          </div>
          {DUTY_DOCTOR_EXAM_TEXT_FIELDS.map(({ key, label }) => (
        ...
        <p className="mt-4 text-xs text-slate-500">
          Saqlangan: {formatDutyDoctorExaminationSavedAt(activeRecord.updatedAt)}
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-2xl border border-sky-200/70 bg-white p-5 shadow-sm">
          ...
          <div>
            <div className="flex items-center gap-2 text-slate-800">
              ...
              <h3 className="font-semibold">
                {editingId ?
                  'Navbatchi shifokor ko\'rig\'ini tahrirlash'
                : 'Yangi navbatchi shifokor ko\'rigi'}
              </h3>
            </div>
            <p className="text-sm text-slate-500">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="grid gap-1.5">
            <Label htmlFor="duty-exam-date">{DUTY_DOCTOR_EXAM_FIELD_LABELS.examinationDate}</Label>
            <Input
              id="duty-exam-date"
              type="date"
          ...
          <div className="grid gap-1.5">
            <Label htmlFor="duty-exam-time">{DUTY_DOCTOR_EXAM_FIELD_LABELS.examinationTime}</Label>
            <Input
              id="duty-exam-time"
              type="time"
          ...
          <div className="grid gap-1.5 md:col-span-2">
            <Label>{DUTY_DOCTOR_EXAM_FIELD_LABELS.receptionDoctor}</Label>
            <Select
          ...
          {DUTY_DOCTOR_EXAM_TEXT_FIELDS.map(({ key, label, placeholder, rows = 4 }) => (
            <div key={key} className="grid gap-1.5 md:col-span-2">
              <Label htmlFor={`duty-exam-${key}`}>{label}</Label>
              <Textarea
                id={`duty-exam-${key}`}
                value={draft[key]}
      ...
      {records.length > 0 ?
        <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div>
              <h4 className="font-semibold text-slate-900">Oldingi navbatchi shifokor ko&apos;rig&apos;ilari</h4>
          ...
          <div className="space-y-3">
            {records.map((record) => (
              <button
                key={record.id}
                type="button"
                className="w-full rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3 text-left transition hover:border-sky-200 hover:bg-sky-50/40"
                onClick={() => openView(record)}>
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <p className="font-medium text-slate-900">{formatDutyDoctorExaminationTitle(record)}</p>
                    <p className="mt-1 text-sm text-slate-600">
                      {record.receptionDoctorName || 'Shifokor tanlanmagan'}
                    </p>
                  </div>
                  <span className="text-xs text-slate-500">
                    {formatDutyDoctorExaminationSavedAt(record.updatedAt)}
                  </span>
                </div>
                {record.primaryDiagnosis ?
                  <p className="mt-2 line-clamp-2 text-sm text-slate-700">{record.primaryDiagnosis}</p>
                : record.recommendation ?
                  <p className="mt-2 line-clamp-2 text-sm text-slate-700">{record.recommendation}</p>
                : null}
              </button>
            ))}
          </div>
