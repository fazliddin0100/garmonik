...
export function PaymentQueuePanel({ selectedQueueId, onSelect, onClear }: Props) {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-6 text-sm text-muted-foreground">
  ...
  if (error) {
    return (
      <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-4 text-sm text-rose-800">
        {error}
        <Button type="button" variant="ghost" size="sm" className="ml-2" onClick={() => void loadQueue()}>
          Qayta
        </Button>
      </div>
    );
    ...
    return (
      <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Kabinet, shifokor buyurtmalari va klinikaga yotqizish to&apos;lovlari shu yerda paydo
          bo&apos;ladi
        </p>
      </div>
    );
    ...
    <div className="space-y-3 sm:col-span-2">
      <div className="flex items-center justify-between gap-2">
        <p className="text-sm font-medium text-slate-700">
          To&apos;lov navbati <span className="text-muted-foreground">({items.length})</span>
        </p>
        <Button type="button" variant="ghost" size="sm" onClick={() => void loadQueue()}>
          Yangilash
        </Button>
      </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    {item.paymentKind === 'post_doctor' && typeof item.postPaymentTotal === 'number' && item.postPaymentTotal > 0 ? (
